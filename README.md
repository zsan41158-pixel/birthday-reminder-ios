...
| 农历转换 | 自建 `lunar_service.dart`（无需外部依赖） |
...
1. **通知权限**：首次使用需在设置页或系统弹窗中授权通知权限，否则无法收到提醒。
2. **农历通知策略**：iOS 本地通知不支持"每年农历 X 月 X 日"的重复规则，App 会在启动时自动计算未来 2 年的农历生日对应公历日期并批量注册通知。建议保持 App 定期前台运行，以便补充未来年份的通知。
3. **真机测试**：通知功能在 iOS 模拟器上可能表现不完整，建议在真机上测试。
